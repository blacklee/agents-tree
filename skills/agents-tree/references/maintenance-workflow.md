...
When current design intent is unclear and would change routing, boundary, skip, or verification guidance, record a short `Clarification Needed` section in the nearest managed guidance artifact. Do not turn uncertainty into vague generated guidance.

Use `Clarification Needed` as a lightweight unresolved reminder, not as a task tracker:

- Record only the unclear boundary or rule, the affected decision, the question for the user, and what agents should do until clarification.
- Keep `Recent touches` to the latest 3-5 dates in `YYYY-MM-DD` form.
- Append a touch date only when the current task actually encounters that ambiguity and it affects the next action. Do not update touches merely because the file was read.
- If the same unresolved clarification is encountered after a reasonable interval, mention it to the user again. If it was touched recently and does not block the current task, avoid repeating the reminder.
- After the user clarifies the intent, delete the `Clarification Needed` section, turn the result into concrete generated guidance, and record the user-provided context in `Evidence Notes`.
...
- Does it record a concise `Clarification Needed` section when current design intent is too ambiguous to guide the next action?
